...
class gwnet(nn.Module):
    def __init__(self, device, num_nodes, dropout=0.3, supports=None, gcn_bool=True, addaptadj=True, aptinit=None, in_dim=2,out_dim=12,residual_channels=32,dilation_channels=32,skip_channels=256,end_channels=512,kernel_size=2,blocks=4,layers=2, incident=False, weather=False, graph_emb_update=False, graph_emb=None):
        ...
        if self.graph_emb is not None:
            self.start_conv = nn.Conv2d(in_channels=in_dim+self.graph_emb.shape[1],
                                        out_channels=residual_channels,
                                        kernel_size=(1,1))
            self.node_embed = nn.Parameter(self.graph_emb,requires_grad=self.graph_emb_update)
        else:
    ...
    def forward(self, input, incident_x=None, weather_x=None):
        ...
        if self.graph_emb is not None:
            x = x.transpose(1,3)
            B,L,_,_ = x.size()
            x = torch.cat([x,self.node_embed.unsqueeze(0).unsqueeze(1).expand(B,L,-1,-1)], dim=-1)
            x = x.transpose(3,1)
            x = self.start_conv(x)
        else:
            x = self.start_conv(x)
        ...
        x = self.end_conv_2(x)
        return x
# CUDA_VISIBLE_DEVICES=1 python train.py --device cuda:0 --gcn_bool --adjtype doubletransition --addaptadj  --randomadj  --epoch 100 --expid 5  --save ./experiment/temp --nhid 128 --graph_embedding True
